...
type PickupStudentStatus = "pending" | "pickedUp" | "absent";

type PickupStudent = Omit<Student, "status"> & {
  status: PickupStudentStatus;
};
...
export default function PickupScreen() {
  ...
  const studentList = useMemo<PickupStudent[]>(
    () =>
      storeStudents
        .filter(
          (student) =>
...
const currentLocation =
  ...
  const colors = {
    ...
    softBackground: isDarkMode ? "#102D46" : "#F7FAFD",
    inactiveBackground: isDarkMode ? "#27405B" : "#EEF3F8",
    successBackground: isDarkMode ? "#123837" : "#E7F6F4",
    successText: isDarkMode ? "#76E2D8" : "#0B7C76",
    absentBackground: isDarkMode ? "#4a2020" : "#fee2e2",
    absentText: isDarkMode ? "#fca5a5" : "#b91c1c",
    pendingBackground: isDarkMode ? "#4a4018" : "#fff4c7",
    pendingText: isDarkMode ? "#fde68a" : "#9a6700",
    blueBackground: isDarkMode ? "#163657" : "#EAF3FC",
    blueText: isDarkMode ? "#8EC7FF" : "#1768C4",
  ...
  const locationGroups = useMemo<PickupLocationGroup[]>(() => {
    const groupMap = new Map<string, PickupStudent[]>();

    studentList.forEach((student) => {
      const location = student.pickupLocation || "Location Not Available";
      const current = groupMap.get(location) ?? [];
      groupMap.set(location, [...current, student]);
    });

    return Array.from(groupMap.entries()).map(([location, groupedStudents]) => ({
      location,
      students: groupedStudents,
    }));
  }, [studentList]);

  const pickedUpCount = studentList.filter(
    (student) => student.status === "pickedUp"
  ).length;
  ...
  const markPickedUp = (selectedStudent: PickupStudent) => {
    Alert.alert(
      "Pick Up Student",
      `Mark ${selectedStudent.name} as picked up?`,
      [
        { text: "Cancel", style: "cancel" },
        {
          text: "Pick Up",
          onPress: () => {
            updateStudentStatus(selectedStudent.id, "pickedUp");

            Alert.alert(
              "Picked Up Successfully",
              `${selectedStudent.name} was picked up at ${getCurrentTime()}.\n\nParent message: Your child was picked up by the school bus.`
            );
          },
        },
      ]
  ...
  const markAbsent = (selectedStudent: PickupStudent) => {
    Alert.alert(
      "Mark Student Absent",
  ...
  const markAllStudentsPickedUpAtLocation = (
    location: string,
    groupStudents: PickupStudent[]
  ) => {
  ...
  //     const supported =
  //       await Linking.canOpenURL(mapUrl);
  ...
  //     Alert.alert(
  //       "Map Error",
  //       "Unable to open this pickup location."
  //     );
  //   }
  // };
...
const showLocation = async (location: string) => {
 ...
 const confirmEndTrip = () => {
    ...
    Alert.alert(
      ...
      `${pendingCount} student${
        pendingCount === 1
          ? ""
          : "s"
      } still pending. Mark each student as picked up or absent first.`
    );
  ...
  try {
    ...
    Alert.alert(
      "Trip Completed",
      "Pickup trip completed successfully.",
      [
        {
          text:
            "OK",
    ...
    >
      <View style={styles.header}>
        ...
        <View style={styles.headerContent}>
          <Text style={[styles.heading, { color: colors.text }]}>Pickup Trip</Text>
        </View>
        ...
        <View style={styles.routeCard}>
          <View style={styles.routeTopRow}>
            ...
            <View style={styles.routeBusIconContainer}>
              <Text style={styles.routeBusIcon}>🚌</Text>
            </View>
          </View>
  ...
  style={({ pressed }) => [
    ...
    pressed &&
      styles.currentLocationInlinePressed,
  ]}
  onPress={() =>
...
>
  <View
    style={
      styles.currentLocationInlineIcon
    }
  >
    <Text
      style={
        styles.currentLocationInlineEmoji
      }
    >
      📍
    </Text>
  ...
  <View
    style={
      styles.currentLocationInlineContent
    }
  >
    <Text
      style={
        styles.currentLocationInlineLabel
      }
    >
      CURRENT LOCATION
    </Text>
    ...
    <Text
      style={
        styles.currentLocationInlineName
      }
    >
      {currentLocation}
    </Text>
  ...
  <Text
    style={
      styles.currentLocationInlineMap
    }
  >
    Map ›
  </Text>
        ...
        {delayMinutes !== null && (
          ...
          >
            <Text style={styles.delayNoticeIcon}>⏱️</Text>
            ...
            return (
              ...
              >
                ...
                >
                  ...
                  >
                    <Text
                      style={[styles.stopNumberText, { color: colors.blueText }]}
                    >
                      {groupIndex + 1}
                    </Text>
                  </View>
                  ...
                  <View style={styles.stopHeaderContent}>
                    ...
                    >
                      ...
                      {groupPickedCount > 0 ? ` • ${groupPickedCount} picked` : ""}
                  ...
                  >
                    <Text style={styles.stopMapIcon}>📍</Text>
                  </Pressable>
                  ...
                  <Pressable
                    ...
                    onPress={(event) => {
                      event.stopPropagation();
                      markAllStudentsPickedUpAtLocation(
                        group.location,
                        group.students
                      );
                    }}
                ...
                {isExpanded && (
                  <>
                    ...
                    {group.students.map((student, index) => {
                      const isPickedUp = student.status === "pickedUp";
                      ...
                      return (
                        ...
                        >
                          ...
                          <Pressable
                            ...
                            onPress={() =>
                              router.push(
                                `/student-details?studentId=${student.id}&mode=pickup`
                              )
                            }
                          >
                            <Text
                          ...
                          {!isPickedUp && !isAbsent ? (
                            <View style={styles.timelineActions}>
                              ...
                              <Pressable
                                ...
                                onPress={() => markPickedUp(student)}
                              >
                                <Text style={styles.timelinePickupIcon}>✓</Text>
                              </Pressable>
                            </View>
                          ) : (
                            <Pressable
                              ...
                              onPress={() => resetStudentStatus(student.id)}
                            >
                              <Text
                                style={[
